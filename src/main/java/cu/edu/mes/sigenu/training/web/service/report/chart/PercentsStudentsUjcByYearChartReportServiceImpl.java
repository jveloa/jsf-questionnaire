...
@Named
@RequestScoped
public class PercentsStudentsUjcByYearChartReportServiceImpl implements PercentsStudentsUjcByYearChartReportService {
    ...
    public Double percentsStudentsUjcByYear(Integer year, Integer questionnarieId) {
        ...
        try {
            MultiValueMap<String, String> params = new LinkedMultiValueMap<>();
            ApiRestMapper<Double> apiRestMapper = new ApiRestMapper<>();

            UriTemplate template = new UriTemplate("/api/v1/training/reportCareerChief/percentsStudentsUjcByYear/{year}/{questionnarieId}");
            String uri = template.expand(year, questionnarieId).toString();

            String response = (String) restService.GET(uri, params, String.class, CurrentUserUtils.getTokenBearer()).getBody();
            items = apiRestMapper.mapOne(response, Double.class);
        } catch (IOException e) {
            e.printStackTrace();
        }
        return items;
